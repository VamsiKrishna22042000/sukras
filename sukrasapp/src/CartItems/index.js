...
const pageStage={
    ...
    const deleteServiceFromCart = async(event) =>{
        ...
        const filterTimeArr = ArrayWithTime.filter(each=>each.timeId!==TimeId[0].timeId)
    ...
    const sendDates = (dates) =>{
       ...
       return dateRange.map((date) => (
            <option selected = {dates === formatDate(date) ? true : false}  key={formatDate(date)} value={formatDate(date)}>
                {formatDate(date)}
            </option>
        ))
    ...
    const getAvailableSlots = (booked) =>{
        ...
        const timesAvailable = ["10:00","11:00","12:00","01:00","02:00", "03:00", "04:00", "05:00", "06:00", "07:00", "08:00", "09:00"]
    ...
    useEffect( ()=>{
        ...
        cartArr.map(async (each)=>{
            ...
            if(response.ok){ 
                    ...
                    setDisplayArray(prevDisplay =>([...prevDisplay,{...each,date:formatDate(moment()._d),time:"",timeId:uuidv4(),availableSlots:getAvailableSlots(data.bookedSlots)}]))                   
            }
     })
    ...
    const setDatesSelected =   (event) =>{
       ...
       const promiseDisplay = displayArr.map( async each =>{
        ...
        promiseDisplay.map(each=>each.then(result=>{
            if(result!==undefined){
                setDisplayArray([])
                displayArr.map(each=>{
    ...
    const setTimesSelected = (event) =>{
        ...
        if(includes[0] !== undefined ){
            ArrayWithTime.map(eachI=>{
                ...
                if(eachI.timeId === idSent){
                    const filteredArrObj = (ArrayWithTime.filter(each=>each.timeId!==idSent))
            ...
            const updatedArray = ArrayWithTime.map((eachItem) => {
                if (eachItem.timeId === idSent) {
                  ...
                  const filteredObj = filteredArr.filter(each=>each!==undefined)
            ...
            displayArr.map(each=>{
                const {time,...rest}=each  
                if(each.timeId === idSent ){
                    setArrayWithTime(prevTimeArr=>([...prevTimeArr,{...rest,time:timeSent}]))
                 }
        ...
        if(ArrayWithTime.length >= displayArr.length){
            ...
            displayArr.map(each=>{
                const {time,date,...rest}=each  
                if(each.timeId === idSent ){
                    setArrayWithTime(prevTimeArr=>([...prevTimeArr,{...rest,date:sentdate,time:timeSent}]))
                 }
    ...
    const getTheTimeSlots = async (obj) =>{
            ...
            if(response.ok){
                /*console.log(data)*/
               return {date,image,price,serviceId,service,_id,time:"",timeId,availableSlots:getAvailableSlots(data.bookedSlots)}
            }
        ...
        <>
                ...
                <div className='cart-item'>
                        ...
                        <div className='cart-contents'>
                            ...
                            <div className='slots-available'>
                                    <select  id={each._id} onChange={setDatesSelected} className='select-options1'>
                                        {sendDates(each.date)}
                                    </select>  
                                    <select id={each.timeId} onChange={setTimesSelected}  className='select-options2'>
                                       {each.availableSlots.map(each=>(<option>{each}</option>))}        
                                    </select>
            ...
            <div className='price-details'> 
                ...
                <div style={{display:"flex",flexDirection:"row",justifyContent:'space-between'}}>
                    ...
                    <p className='price1'><span className='actual-price'>₹</span>{TotalPrice}</p>
                </div>
                <div style={{display:"flex",flexDirection:"row",justifyContent:'space-between'}}>
                    <p className='total-discount'>Total Discount</p>
                    <p className='total-discount'><span className='actual-price'>-₹</span>{discount}</p>
                </div>
                <div className='total-cart-price' style={{display:"flex",flexDirection:"row",justifyContent:'space-between'}}>
                    ...
                    <p className='total-price-head'><span className='actual-price'>₹</span> {TotalPrice-discount}</p>
                </div>
                <button onClick={sendUpdate} className='proceed-schedule' type="button">Proceed</button>
            </div>
        </>
    )
}

...
const ScheduleTime  = (props) =>{
    ...
    const sendUpdate = (props)=>{
        if(buttonSelected !== ""){
           ...
           alert(`Select Cash After Service`)
        }
        ...
        <div className='price-details'> 
                ...
                <div className='payment-mode'>
                  ...
                  {paymentMethodArr.map(each=>(
                    <button onClick={selectButton} className={each.mode !== "Cash After Service" ? "payment-modebutton2" :buttonSelected==="" ?"payment-modebutton1" : "payment-modebutton3" } type="button"><img className='paymentmode-image' src={`/${each.imgUrl}.png`} alt={each.mode}/><p>{each.mode}</p><img className={each.mode !== "Cash After Service" ? "yes-disable" : buttonSelected=== "" ? "yes-disable" : "yes-enable"  } src={`/yes.png`} alt ={each.yes}/></button>
                  ))
                  }
                </div>

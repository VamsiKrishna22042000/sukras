...
const FashionCart = (props) =>{
   ...
   const getAllCartItems = async() =>{
    ...
    const gobackTo = () =>{
        ...
        if(params.zone==="fashionzone"){
           history.replace(`/${params.zone}`)
            ...
            history.replace(`/fashioncategory/detailedview/${params.type}/${params.name}/${params.id}`)
        }
         else{
    ...
    const updateProgress = (value) =>{
     ...
     return(
        load ? cartItems === [] ? <div className='loader-spinner'>
      ...
      <>
        <div className='payment-progress'>
            ...
            <div className='progress'>
                    <p className='progress-para1'>{progress==="Cart"? 1:<BsCheck2 className='check-mark'/>}</p>
                    <p className='progress-para2'>Cart</p>
                </div>
                <p className='progress-bar1'> - - - - - - - - - - - - - - -</p>
                <div className='progress'>
                    <p className={progress==="Cart"?'progress-para15':"progress-para3"}>{progress==="Payment"? 2 :<BsCheck2 className='check-mark'/>}</p>
        ...
        <div className='price-details'> 
                ...
                <button onClick={updateProgress} className={progress === "Cart" ?'fashion-proceed-to-schedule' : "fashion-proceed-button-disable"} type="button">Proceed</button>
        </div>
        </div>
     </> : <div className='loader-spinner'><TailSpin color={"#F4BD18"} height={70} width={70}/></div>
     )
}
